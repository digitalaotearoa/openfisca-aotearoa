...
class social_security__meets_residential_requirements_for_certain_benefits(Variable):
    ...
    def formula_2018_01_01(persons, period, parameters):
        ...
        nz_eligible = persons('social_security__is_ordinarily_resident_in_new_zealand', period) * person('social_security__has_resided_continuously_in_nz_for_a_period_of_at_least_2_years_at_any_one_time', period) 
        reciprocal_eligible = person("social_security__is_ordinarily_resident_in_country_with_reciprocity_agreement", period) * (person('years_resided_continuously_in_new_zealand', period) > 2)
